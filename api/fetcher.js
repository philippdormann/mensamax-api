...
function getCacheItem(key) {
  return new Promise(function(resolve, reject) {
    if (process.env.CACHE === 'redis') {
      ...
      const expiry = Date.now() - process.env.CACHE_TIME_MINUTES * 60 * 1000;
      const cacheItem =
          mensaplanCache.find((i) => i.ts > expiry && i.key === key);
      if (cacheItem) {
        resolve(cacheItem);
      } else {
        resolve(undefined);
...
function getMensaplanHTML({p, e}) {
  return new Promise(function(resolve, reject) {
    if (p && e) {
      const found = institutions.find(function(
          ins) { return ins.project === p && ins.facility === e; });
      if (found) {
        if (process.env.CACHE === 'none') {
          fetchHTML({p, e, provider : found.provider})
          ...
          getCacheItem(`${p}${e}${found.provider}`)
              .then((cache) => {
                if (cache) {
                  ...
                  // load fresh data
                  fetchHTML({p, e, provider : found.provider}).then((data) => {
                    updateCacheItem(`${p}${e}${found.provider}`, data);
                    resolve(data);
                  });
                }
              })
    ...
    axios
        ...
        .then(function(response) {
          ...
          request({
            ...
            qs : {p, e},
            formData : {__VIEWSTATE, __VIEWSTATEGENERATOR, btnLogin : ''}
          },
                  (error, response, body) => {
                    if (error) {
                      reject('fetch_step2');
        ...
        .catch(function(error) { reject('fetch_step1'); });
  });
}
...
function recursiveFutureWeek({body, p, e, provider, weekstogo}) {
  return new Promise(function(resolve, reject) {
           ...
           request({
             ...
             method : "POST",
             url :
                 `https://mensahaus.de/mensamax/Wochenplan/WochenplanExtern/WochenPlanExternForm.aspx`,
             qs : {p, e},
             formData : {
               __VIEWSTATE,
